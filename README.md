...
- The `LegalParticipant` credential referenced in the Trust Framework 22.10 has a different shape one than the one that is accepted by the current (Compliance Service)[https://compliance.lab.gaia-x.eu/main/docs] . As we stick completely to the 22.10 version in this example, the credential isn't accepted by the Compliance Service and we are also missing the credentials for `legalRegistrationNumber` and `TermsAndConditions`.
